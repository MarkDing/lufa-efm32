...
/** \file
 *  \brief USB Endpoint definitions for the EFM32 Giant Gecko microcontrollers.
 *  \copydetails Group_EndpointManagement_EFM32GG
 *
 ...
 *  \defgroup Group_EndpointRW_EFM32GG Endpoint Data Reading and Writing (EFM32GG)
 *  \brief Endpoint data read/write definitions for the Silabs EFM32 Giant Gecko architecture.
 *
 *  Functions, macros, variables, enums and types related to data reading and writing from and to endpoints.
 */

/** \ingroup Group_EndpointPrimitiveRW
 *  \defgroup Group_EndpointPrimitiveRW_EFM32GG Read/Write of Primitive Data Types (EFM32GG)
 *  \brief Endpoint primitive read/write definitions for the Silabs EFM32 Giant Gecko architecture.
 *
 ...
 *  \defgroup Group_EndpointPacketManagement_EFM32GG Endpoint Packet Management (EFM32GG)
 *  \brief Endpoint packet management definitions for the ilabs EFM32 Giant Gecko architecture.
 *
 *  Functions, macros, variables, enums and types related to packet management of endpoints.
 */

/** \ingroup Group_EndpointManagement
 *  \defgroup Group_EndpointManagement_EFM32GG Endpoint Management (EFM32GG)
 *  \brief Endpoint management definitions for the Silabs EFM32 Giant Gecko architecture.
 *
...
#ifndef __ENDPOINT_EFM32GG_H__
#define __ENDPOINT_EFM32GG_H__
...
#define INLINENON inline
#define	ATTR_ALWAYS_INLINE2
/* Function Prototypes: */
 ...
 *  be used in the device. Different EFM32 models support different amounts of endpoints,
 *  this value reflects the maximum number of endpoints for the currently selected EFM32 model.
 */
 ...
 *
 *  \ingroup Group_EndpointRW_EFM32GG
 */
...
static INLINENON uint16_t Endpoint_BytesInEndpoint(void)
{
	if (ep[ep_selected].remaining < ep[ep_selected].packetSize) {
		return (uint16_t)ep[ep_selected].remaining;
	} else {
		return (uint16_t)ep[ep_selected].packetSize;
	}
}
...
{
	return (ep[ep_selected].in ? ENDPOINT_DIR_IN : ENDPOINT_DIR_OUT);
}
...
{
	return (ep_selected | Endpoint_GetEndpointDirection());
}
...
{
	ep_selected = (Address & ENDPOINT_EPNUM_MASK);
}
...
{
	uint32_t num = (Address & ENDPOINT_EPNUM_MASK);
	if (ep[num].in) {
		USB->GRSTCTL = USB_GRSTCTL_RXFFLSH;
		while (USB->GRSTCTL & USB_GRSTCTL_RXFFLSH) {}
	} else {
		USB->GRSTCTL = USB_GRSTCTL_TXFFLSH | (ep[num].txFifoNum <<
		                                      _USB_GRSTCTL_TXFNUM_SHIFT);
...
{
	if (ep[ep_selected].in) {
		USB_DINEPS[ep_selected].CTL |= USB_DIEP_CTL_CNAK | USB_DIEP_CTL_EPENA;
	} else {
		USB_DOUTEPS[ep_selected].CTL |= USB_DOEP_CTL_CNAK | USB_DOEP_CTL_EPENA;
	}
}
...
{
	if (ep[ep_selected].in) {
		USB_DINEPS[ep_selected].CTL |= USB_DIEP_CTL_EPDIS;
	} else {
		USB_DOUTEPS[ep_selected].CTL |= USB_DOEP_CTL_EPDIS;
	}
}
...
{
	if (ep[ep_selected].in) {
		return (USB_DINEPS[ep_selected].CTL & USB_DIEP_CTL_USBACTEP);
	} else {
		return (USB_DOUTEPS[ep_selected].CTL & USB_DOEP_CTL_USBACTEP);
	}
}
 ...
 *
 *  \ingroup Group_EndpointPacketManagement_EFM32GG
 *
...
{
	return (ep[ep_selected].packetSize ? true : false);
}
...
{
	return (USB->DAINT & USB->DAINTMSK);
}

/** Determines if the specified endpoint number has interrupted (valid only for INTERRUPT type
 *  endpoints). Endpoint out interrupt start from bit 16, Endpoint in interrupt start from 0.
 *
...
{
	if (ep[Address].in) {
		return (Endpoint_GetEndpointInterrupts() & (Address & ENDPOINT_EPNUM_MASK)) ? true : false;
	} else {
		return (Endpoint_GetEndpointInterrupts() & (Address & ENDPOINT_EPNUM_MASK) << 16) ? true : false;
	}
}
...
{
	if ((USB_DINEPS[ep_selected].CTL & USB_DIEP_CTL_EPENA) == 0) {
		if (USB_DINEPS[ep_selected].INT & USB_DIEP_INT_XFERCOMPL) {
			USB_DINEPS[ep_selected].INT |= USB_DIEP_INT_XFERCOMPL;
		}
		return true;
	}
	return false;
...
{
	if (USB_DOUTEPS[ep_selected].INT & USB_DOEP_INT_XFERCOMPL) {
		USB_DOUTEPS[ep_selected].INT |= USB_DOEP_INT_XFERCOMPL;
		return true;
	}
	return false;
}
...
{
	bool status = false;
	if (ep_selected == ENDPOINT_CONTROLEP) {
		status = true;
	} else {
		if (ep[ep_selected].in) {
...
{
	return (USB_DOUTEPS[ENDPOINT_CONTROLEP].INT & USB_DOEP_INT_SETUP) ? true : false;
}
...
{
	ep->remaining = 0;
	USB_Endpoint_FIFOPos[ep_selected] = ep->buf;

	USB_DOUTEPS[ENDPOINT_CONTROLEP].INT |= USB_DOEP_INT_SETUP;
 ...
 *  freeing up the endpoint for the next packet.
 *
 *  \ingroup Group_EndpointPacketManagement_EFM32GG
 */
...
{
	USBD_Ep_TypeDef *ep = &dev->ep[ep_selected];
...
{
	USBDHAL_StallEp(&ep[ep_selected]);
}
...
{
	USBDHAL_UnStallEp(&ep[ep_selected]);
}
...
{
	return USBDHAL_EpIsStalled(&ep[ep_selected]);
}
...
{
	if (ep[ep_selected].in) {
		USB_DINEPS[ep_selected].CTL |= USB_DIEP_CTL_SETD0PIDEF;
	} else {
		USB_DOUTEPS[ep_selected].CTL |= USB_DOEP_CTL_SETD0PIDEF;
	}
}
...
{
	ep[ep_selected].in = (DirectionMask == ENDPOINT_DIR_IN) ? true : false;
}

/** Reads one byte from the currently selected endpoint's bank, for OUT direction endpoints.
 *
 *  \ingroup Group_EndpointPrimitiveRW_EFM32GG
 *
...
{
	return *(USB_Endpoint_FIFOPos[ep_selected]++);
}
...
{
	ep[ep_selected].remaining++;
	*(USB_Endpoint_FIFOPos[ep_selected]++) = Data;
}
...
static INLINENON void Endpoint_Discard_8(void)
{
	...
	Dummy = *(USB_Endpoint_FIFOPos[ep_selected]++);
...
{
	uint16_t Byte0 = *(USB_Endpoint_FIFOPos[ep_selected]++);
	uint16_t Byte1 = *(USB_Endpoint_FIFOPos[ep_selected]++);
...
{
	ep[ep_selected].remaining += 2;
	*(USB_Endpoint_FIFOPos[ep_selected]++) = (Data & 0xFF);
	*(USB_Endpoint_FIFOPos[ep_selected]++) = (Data >> 8);
}
...
{
	uint32_t Byte0 = *(USB_Endpoint_FIFOPos[ep_selected]++);
	uint32_t Byte1 = *(USB_Endpoint_FIFOPos[ep_selected]++);
	uint32_t Byte2 = *(USB_Endpoint_FIFOPos[ep_selected]++);
	uint32_t Byte3 = *(USB_Endpoint_FIFOPos[ep_selected]++);
...
{
	ep[ep_selected].remaining += 4;
	*(USB_Endpoint_FIFOPos[ep_selected]++) = (Data &  0xFF);
	*(USB_Endpoint_FIFOPos[ep_selected]++) = (Data >> 8);
	*(USB_Endpoint_FIFOPos[ep_selected]++) = (Data >> 16);
	*(USB_Endpoint_FIFOPos[ep_selected]++) = (Data >> 24);
}
